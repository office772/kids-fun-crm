...
function BotSimulator() {
  const [messages, setMessages] = useState<{ role: 'user' | 'bot'; text: string }[]>([
    {
      role: 'bot',
      text: 'שלום! 😊 כאן Kids & Fun! איך אפשר לעזור?\n\n*1* — רישום לצהרון\n*2* — רישום לקייטנה\n*3* — ביטול\n*4* — שעות ולוח זמנים\n*5* — תשלומים\n*6* — איסוף מוקדם\n\nאו פשוט כתוב/י מה צריך 💬',
    },
  ])
  const [input, setInput] = useState('')
  const [botLoading, setBotLoading] = useState(false)
  const sessionId = 'simulator_' + (typeof window !== 'undefined' ? Date.now() : '1')
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      setMessages(prev => [...prev, { role: 'bot', text: data.reply || 'שגיאה בתשובה' }])
    } catch {
  ...
  return (
    <div>
      ...
      <div
        className="bg-[#E5DDD5] rounded-2xl overflow-hidden"
        style={{
          backgroundImage:
            'url("data:image/svg+xml,%3Csvg width=\'60\' height=\'60\' viewBox=\'0 0 60 60\' xmlns=\'http://www.w3.org/2000/svg\'%3E%3Cg fill=\'none\' fill-rule=\'evenodd\'%3E%3Cg fill=\'%23C4B9B0\' fill-opacity=\'0.2\'%3E%3Cpath d=\'M36 34v-4h-2v4h-4v2h4v4h2v-4h4v-2h-4zm0-30V0h-2v4h-4v2h4v4h2V6h4V4h-4zM6 34v-4H4v4H0v2h4v4h2v-4h4v-2H6zM6 4V0H4v4H0v2h4v4h2V6h4V4H6z\'/%3E%3C/g%3E%3C/g%3E%3C/svg%3E")',
        }}
      >
        <div className="bg-[#128C7E] text-white px-4 py-3 flex items-center gap-3">
          ...
          <div>
            <p className="font-bold">Kids &amp; Fun</p>
            <p className="text-green-200 text-xs">בוט שירות</p>
          </div>
        </div>
        ...
        <div className="p-4 h-96 overflow-y-auto space-y-3">
          {messages.map((msg, i) => (
            <div key={i} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
              <div
                className={`max-w-[80%] rounded-2xl px-4 py-3 shadow-sm text-sm whitespace-pre-line ${
                  msg.role === 'user'
          ...
          {botLoading && (
            <div className="flex justify-start">
              <div className="bg-white rounded-2xl px-4 py-3 shadow-sm text-stone-400 text-sm animate-pulse">
                מקלידה...
              </div>
            </div>
          )}
        ...
        <div className="bg-[#F0F0F0] p-3 flex items-center gap-2">
          ...
          <button
            ...
            className="bg-[#128C7E] hover:bg-[#0e7268] disabled:opacity-50 text-white rounded-full w-10 h-10 flex items-center justify-center transition-colors text-xl"
          >
            ←
          </button>
        </div>
      ...
      <p className="text-stone-400 text-xs mt-3 text-center">
        💡 נסי: &quot;אני רוצה להירשם לצהרון&quot;, &quot;שעות הצהרון&quot;, &quot;יש לי בעיה עם התשלום&quot;, &quot;ביטול&quot;
      </p>
    </div>
  )
}
