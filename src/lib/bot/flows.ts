...
// astra סבב 12 (P2): "וגם" במקום כלשהו + מסגרת + תשלום אינם "שניהם" ("את התשלום לצהרון וגם לקייטנה"
//   מחבר שתי מסגרות של התשלום). **אין היקש**: רק ביטוי שלם שבו "וגם" מחבר *מילת-רישום* עם
//   *מילת-תשלום* (בשני הסדרים, באותה פסוקית). כל שילוב אחר → בירור.
const CANCEL_PAIR_RE = /(רישום|הרשמה|צהרון|קייטנה)[^,.;!?]*וגם[^,.;!?]*(הוראת\s*ה?קבע|הו\s*ק|תשלום)|(הוראת\s*ה?קבע|הו\s*ק|תשלום)[^,.;!?]*וגם[^,.;!?]*(רישום|הרשמה)(\s|$)/
...
export async function handlePaymentSetupFlow(
  ...
  if (step === 'payment_setup_cancel_choice') {
    ...
    // "שניהם"/"הכל" מפורש, או ביטוי שלם "רישום וגם תשלום" (CANCEL_PAIR_RE) → ביטול רישום
    //   (מבטל ממילא גם את הו"ק ב-PayPlus; יש שלב אישור נפרד לפני ביצוע).
    //   astra סבבים 11-12: אין היקש משתי מילות-מטרה ("את הוראת הקבע לצהרון") ולא מ-"וגם" חופשי
    //   ("את התשלום לצהרון וגם לקייטנה"). שילוב שאינו הביטוי השלם → לא מוכרע → בירור שוב.
    const explicitPair = reg === 'yes' && pay === 'yes' && CANCEL_PAIR_RE.test(core)
