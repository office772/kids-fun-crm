...
//   astra חלק ב' (3): **התאמה מלאה** לביטויי אישור (כולל טבעיים כמו "כן אשמח"/"בשמחה")
//   — *לא* תחילת-משפט/הגבלת-אורך. משפט קצר אינו בהכרח אישור ("רוצה לחשוב"/"כן בהמשך").
const CONFIRM_STANDING = [
  'כן', 'כן בבקשה', 'כן להסדיר', 'להסדיר', 'הוראת קבע', 'כן הוראת קבע', 'הוראת קבע בבקשה', 'קבע',
  'מאשר', 'מאשרת', 'אישור',
  'כן אשמח', 'אשמח', 'בשמחה', 'בטח', 'סבבה', 'אוקיי', 'אוקי', 'בסדר', 'בסדר גמור',
  'מעולה', 'יאללה', 'נשמע טוב', 'לגמרי', 'בהחלט', 'קדימה',
  'רוצה', 'אני רוצה', 'כן רוצה', 'כן אני רוצה', 'רוצה הוראת קבע', 'אני רוצה הוראת קבע',
  'כן רוצה הוראת קבע', 'בוא נעשה', 'בוא נעשה הוראת קבע', 'נעשה הוראת קבע',
] as const
...
// astra חלק ב' (3): **אוצר-אישור סגור.** מקבלים כאישור *רק* אם *כל* מילות ההודעה שייכות
//   לאוצר — כך צירוף חופשי ("כן בטח"/"בוא נעשה הוראת קבע") מתקבל, אבל מילה זרה אחת
//   ("רוצה *לחשוב*"/"כן *בהמשך*"/"אני *צריך*") שוברת את האישור → הבהרה. אין ניחוש
//   לפי תחילת-משפט או אורך; ברירת המחדל הבטוחה היא לא-לאשר.
// ⚠️ לא לכלול מילים שמורות (תפריט/סגור/יציאה/ביטול) — ה-handler חוטף אותן כפקודות מערכת.
const AFFIRM_VOCAB = new Set([
  'כן', 'בטח', 'בשמחה', 'אשמח', 'מעולה', 'סבבה', 'אוקיי', 'אוקי', 'מצוין', 'מושלם',
  'בהחלט', 'לגמרי', 'יאללה', 'קדימה', 'רוצה', 'אני', 'מאשר', 'מאשרת', 'אישור', 'מסכים', 'מסכימה',
  'להסדיר', 'הוראת', 'קבע', 'בוא', 'נעשה', 'בסדר',
])

// §10-11: מסווג תגובה להצעת הו"ק. **סדר קדימות: שאלה/בקשת-הסבר → סירוב → אישור → עמום.**
//   כל קלט לא-מסווג נופל ל-unclear (הבהרה) — הצד הבטוח (לא לאשר תשלום בטעות).
function offerVerdict(msg: string): 'accept' | 'refuse' | 'question' | 'unclear' {
  ...
  // 3. אישור: ביטוי קנוני מלא, *או* הודעה שכל מילותיה מאוצר-האישור (בלי qualifier).
  if (unambiguousMatch(msg, CONFIRM_STANDING)) return 'accept'
  if (!CONFIRM_QUALIFIER_RE.test(msg)) {
    const toks = core.split(/\s+/).filter(Boolean)
    if (toks.length > 0 && toks.every(t => AFFIRM_VOCAB.has(t))) return 'accept'
  }
