...
- **LLM Fallback**: Codex Haiku (Anthropic API)
...
[FP] כוונה מזוהה (1-6)? → flows.ts
  ↓ לא תפס
[LLM] Codex Haiku → llm-fallback.ts
```
...
| `src/lib/bot/llm-fallback.ts` | Codex API fallback |
...
- [x] Codex Haiku fallback (llm-fallback.ts)
